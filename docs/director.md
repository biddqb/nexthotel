...
2. Một cửa sổ console hiện với banner:
   ```
   ╔════════════════════════════════════════════════╗
   ║  nextHotel server running                      ║
   ║  Local:    http://127.0.0.1:8080               ║
   ║  Network:  http://192.168.1.100:8080           ║
   ║  Share the Network URL with staff devices.     ║
   ╚════════════════════════════════════════════════╝
   ```
3. Ghi lại **Network URL** (địa chỉ kiểu `192.168.x.x`) — đây là địa chỉ các thiết bị
   khác dùng để truy cập.
4. Mở trình duyệt, gõ địa chỉ đó → ứng dụng hiện ra.

Không đóng cửa sổ console. Nếu đóng → máy chủ tắt.
...
1. Dừng máy chủ (đóng cửa sổ console).
2. Backup dữ liệu: copy thư mục dữ liệu (mặc định `C:\Users\Owner\AppData\Local\nextHotel`
   hoặc đường dẫn đã cấu hình) sang nơi khác.
3. Copy code mới vào thư mục `app`, ghi đè.
4. Double-click `build-release.cmd` lại để build lại.
5. Double-click `nexthotel-server.exe` để chạy.

Dữ liệu khách, phòng, đặt phòng không mất — chỉ code được thay.
...
1. Lấy file **`nexthotel-*.sqlite`** mới nhất từ thư mục sao lưu (hoặc từ cloud drive nếu
   đã cấu hình).
2. Cài lại môi trường (Bun + Rust) trên PC thay thế.
3. Build lại ứng dụng.
4. Trước khi chạy lần đầu, **copy file `.sqlite` đó vào thư mục dữ liệu** và **đổi tên
   thành `nexthotel.db`**.
5. Chạy ứng dụng. Tất cả dữ liệu được khôi phục.
